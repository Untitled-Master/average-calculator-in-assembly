;biblioth�que pour assembleur
;ne g�re pas un flot d'entr�e sup�rieur � 1000 caract�res
;asm_io.inc
%define taille_mot 4 ; 4 en 32 bits, 8 en 64 bits

%define taille_entree_buff 1000 ; tampon qui contiendra les caract�res
                                ; lus depuis l'entr�e standard
...
read_int:
    ...
    add edx,[fin_flot]
    add ecx,[debut_flot]
; sauter caract�re blanc tampon jusqu'a non blanc ou vide
loop:
    cmp ecx,edx
    jne  suite
    call tampon
    ...
    jmp loop
suite:
    cmp byte [ecx],10 ; carat�re saut de ligne
    je  suivant
    cmp byte [ecx],' '
    je  suivant
    cmp byte [ecx],9 ; caract�re tabulation
    je  suivant
    jmp lecture_nombre
suivant:
...
;lire nombre suivant
    push ecx
    push entier
    call convertir_str_int; #� changer ,le nombre peut continuer sur le d�but du tampon
...
tampon:
;Convention C
;Demande une saisie et vide l'entr�e standard
;dans le tampon.
;le d�but du flot est � l'adresse *debut_flot+entree_tampon
;la fin du flot � l'adresse fin_flot (�gale � debut flot si flot est vide)
    push eax
    push ebx
    push ecx
    push edx
...
;*****************************************
sprint_int:
;convention C. �crit en base 10 le nombre.(positif n�gatif ou nul)
;renvoie le nombre de caract�re �crit
;arg1 nombre_a ecrire, arg2 place o� �crire.
    push ebp
    mov ebp,esp
    push ebx
    mov eax,0
...
esprint_int:
;convention C.
;�crit en base 10 le nombre positif strictement � �crire.
;renvoie le nombre de caract�re �crit.
;arg1 nombre_a ecrire, arg2 place o� �crire.
    push ebp
    mov ebp,esp
...
;Retourne le nombre de caract�re lue en cas d'erreur
;arg1 endroit o� �crire (4 byte), arg2 endroit o� lire
    push ebp
    mov ebp,esp
    push ebx ; adresse o� lire
    push ecx ; nouveau caract�re
    mov eax,0 ;eax nombre � ajouter
    mov ebx,[ebp+3*taille_mot]
...
;*****************************************
print_char:
;affiche un caract�re contenue dans al
    mov [caractere],al
    push eax
    mov eax,caractere
    call print_string
...
;le caract�re 0 (fin de chaine) ne compte pas
    push ebp
    mov ebp,esp
